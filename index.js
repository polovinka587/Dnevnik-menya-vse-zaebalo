...
const CONFIG = {
    ...
    // Дополнительные массивы для кастомных элементов
    customEmotions: [],
    customTriggers: []
};
...
function initEvents() {
    ...
    // Быстрый выбор эмоций
    const selectAllBtn = document.getElementById('select-all-emotions');
    const deselectAllBtn = document.getElementById('deselect-all-emotions');
...
function getRandomColor() {
    const colors = [
        '#6C63FF', '#FF6584', '#36D1DC', '#FF9800', 
        '#4CAF50', '#9C27B0', '#E91E63', '#00BCD4',
        '#8BC34A', '#FF5722', '#009688', '#3F51B5'
    ];
...
function renderEmotions() {
    ...
    // Объединяем стандартные эмоции и кастомные
    const allEmotions = [...CONFIG.emotions, ...CONFIG.customEmotions];
    ...
    allEmotions.forEach(emotion => {
        ...
        div.innerHTML = `
            ...
            <label for="emotion-${emotion.id}">
                <span>${emotion.emoji || '❤️'} ${emotion.label}</span>
            </label>
        `;
...
function renderTriggers() {
    ...
    // Объединяем стандартные триггеры и кастомные
    const allTriggers = [...CONFIG.triggers, ...CONFIG.customTriggers];
...
function clearForm() {
    ...
    document.getElementById('what-bothers').value = '';
    document.getElementById('victory').value = '';
    document.getElementById('help-actions').value = '';
    document.getElementById('tag').value = '';
    document.getElementById('inspiration').value = '';
    document.getElementById('custom-emotion-input').value = '';
    document.getElementById('custom-trigger-input').value = '';
    
    // Сбрасываем чекбоксы эмоций
    document.querySelectorAll('.emotion-checkbox input').forEach(cb => cb.checked = false);
    
    // Сбрасываем слайдеры к средним значениям
    document.getElementById('physical-state').value = 5;
    ...
    // Сбрасываем настроение к значениям по умолчанию
    document.getElementById('morning-mood').value = 'normal';
    document.getElementById('evening-mood').value = 'normal';
    
    // Оставляем выбранный триггер пустым
...
function addCustomEmotion() {
    ...
    if (emotionText) {
        const emotionId = emotionText.toLowerCase().replace(/\s+/g, '-');
        
        // Проверяем, нет ли уже такой эмоции (в стандартных или кастомных)
        const allEmotions = [...CONFIG.emotions, ...CONFIG.customEmotions];
        
        if (!allEmotions.find(e => e.id === emotionId)) {
            const newEmotion = {
                id: emotionId,
                label: emotionText,
                emoji: '❤️',
            ...
            // Добавляем в кастомные эмоции
            CONFIG.customEmotions.push(newEmotion);
            ...
            showNotification('Эмоция добавлена в список!', 'success');
        } else {
            showNotification('Такая эмоция уже есть в списке', 'info');
        }
    }
}
...
function addCustomTrigger() {
    ...
    if (triggerText) {
        ...
        if (!allTriggers.includes(triggerText)) {
            // Добавляем в кастомные триггеры
            CONFIG.customTriggers.push(triggerText);
            ...
            renderTriggers();
            input.value = '';
            showNotification('Триггер добавлен в список!', 'success');
        } else {
            showNotification('Такой триггер уже есть в списке', 'info');
        }
    }
}
...
function loadData() {
    ...
    if (data) {
        ...
        if (data.config) {
            // Восстанавливаем кастомные эмоции и триггеры
            if (data.config.customEmotions) {
                CONFIG.customEmotions = data.config.customEmotions;
            }
            if (data.config.customTriggers) {
                CONFIG.customTriggers = data.config.customTriggers;
            }
            // Обновляем другие настройки
            Object.assign(CONFIG.settings, data.config.settings || {});
        }
            ...
            <div class="entry-content">
                ...
                <div class="entry-stats">
                    ${entry.emotions.map(emotion => {
                        const allEmotions = [...CONFIG.emotions, ...CONFIG.customEmotions];
                        const emotionData = allEmotions.find(e => e.id === emotion);
                        return emotionData ? 
                            `<span class="stat-badge">${emotionData.emoji || '❤️'} ${emotionData.label}</span>` : '';
                    }).join('')}
...
function updateFilters() {
    ...
    // Объединяем все эмоции для фильтра
    const allEmotions = [...CONFIG.emotions, ...CONFIG.customEmotions];
...
function openEditModal(entryId) {
    ...
    // Объединяем все эмоции и триггеры
    const allEmotions = [...CONFIG.emotions, ...CONFIG.customEmotions];
    const allTriggers = [...CONFIG.triggers, ...CONFIG.customTriggers];
    ...
    formContainer.innerHTML = `
        <div class="edit-form">
            ...
            <div class="form-section">
                ...
                <select id="edit-trigger">
                    ...
                    ${allTriggers.map(trigger => `
                        <option value="${trigger}" ${entry.trigger === trigger ? 'selected' : ''}>
                            ${trigger}
                        </option>
                    `).join('')}
                </select>
            ...
            <div class="form-section">
                ...
                <div class="importance-stars" id="edit-importance-stars">
                    <span class="star" data-value="1">${entry.importance >= 1 ? '★' : '☆'}</span>
                    <span class="star" data-value="2">${entry.importance >= 2 ? '★' : '☆'}</span>
                    <span class="star" data-value="3">${entry.importance >= 3 ? '★' : '☆'}</span>
                    <span class="star" data-value="4">${entry.importance >= 4 ? '★' : '☆'}</span>
                    <span class="star" data-value="5">${entry.importance >= 5 ? '★' : '☆'}</span>
                </div>
                <input type="hidden" id="edit-importance" value="${entry.importance}">
            </div>
            
            <div class="form-section">
                <label>Маленькая победа:</label>
                <input type="text" id="edit-victory" value="${entry.victory || ''}">
            </div>
            
            <div class="form-section">
                <label>Что помогло:</label>
                <textarea id="edit-help-actions">${entry.helpActions || ''}</textarea>
            </div>
            
            <div class="form-row">
                <div class="form-section">
                    <label>Настроение утром:</label>
                    <select id="edit-morning-mood">
                        <option value="good" ${entry.morningMood === 'good' ? 'selected' : ''}>Хорошо</option>
                        <option value="normal" ${entry.morningMood === 'normal' ? 'selected' : ''}>Нормально</option>
                        <option value="tired" ${entry.morningMood === 'tired' ? 'selected' : ''}>Усталость</option>
                        <option value="anxiety" ${entry.morningMood === 'anxiety' ? 'selected' : ''}>Тревога</option>
                        <option value="anger" ${entry.morningMood === 'anger' ? 'selected' : ''}>Злость</option>
                    </select>
                </div>
                <div class="form-section">
                    <label>Настроение вечером:</label>
                    <select id="edit-evening-mood">
                        <option value="good" ${entry.eveningMood === 'good' ? 'selected' : ''}>Хорошо</option>
                        <option value="normal" ${entry.eveningMood === 'normal' ? 'selected' : ''}>Нормально</option>
                        <option value="tired" ${entry.eveningMood === 'tired' ? 'selected' : ''}>Усталость</option>
                        <option value="anxiety" ${entry.eveningMood === 'anxiety' ? 'selected' : ''}>Тревога</option>
                        <option value="anger" ${entry.eveningMood === 'anger' ? 'selected' : ''}>Злость</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-section">
                <label>Тег:</label>
                <input type="text" id="edit-tag" value="${entry.tag || ''}">
            </div>
            
            <div class="form-section">
                <label>Вдохновение:</label>
                <textarea id="edit-inspiration">${entry.inspiration || ''}</textarea>
            </div>
        </div>
    `;
    
    // Инициализируем звезды важности для редактирования
    const stars = formContainer.querySelectorAll('#edit-importance-stars .star');
    const importanceInput = formContainer.querySelector('#edit-importance');
    ...
    const energySlider = formContainer.querySelector('#edit-energy');
    const physicalSlider = formContainer.querySelector('#edit-physical');
...
function saveEdit() {
    ...
    entry.emotions = Array.from(document.querySelectorAll('.edit-form .emotion-checkbox input:checked'))
        .map(cb => cb.value);
    entry.trigger = document.getElementById('edit-trigger').value;
    ...
    entry.importance = parseInt(document.getElementById('edit-importance').value);
    entry.victory = document.getElementById('edit-victory').value;
    entry.helpActions = document.getElementById('edit-help-actions').value;
    entry.morningMood = document.getElementById('edit-morning-mood').value;
    entry.eveningMood = document.getElementById('edit-evening-mood').value;
    entry.tag = document.getElementById('edit-tag').value;
    entry.inspiration = document.getElementById('edit-inspiration').value;
...
Эмоции: ${entry.emotions.map(e => {
    const allEmotions = [...CONFIG.emotions, ...CONFIG.customEmotions];
    const emotionData = allEmotions.find(em => em.id === e);
    return emotionData ? emotionData.label : e;
}).join(', ')}
